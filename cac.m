...
for k = 1 : K
    ...
    for kk = 1 : K
        if kk == k
            ...
            interf = interf + akkps(k, kk);
        end
    end
    interf = rho0 * interf + 1;
    signalpow = rho0 * akkps(k, k);
...
while patidevi > patidevi_comp * 1e-12
    ...
    %     if abs(partiddb_s-partiddb(ky, 1))<partiddb(ky, 1)*1e-2
    %         flag_pp(ky,1) = 1;
    %     else
    %     end
...
for k = 1 : K
    ...
    for kk = 1 : K
        if kk == k
            ...
            interf = interf + abs(akkps(k, kk))* pks(kk, 1);
        end
    end
    interf = interf + 1;
    signalpow = abs(akks(k, 1)) * pks(k, 1);
    sinr = real(signalpow  / interf);
